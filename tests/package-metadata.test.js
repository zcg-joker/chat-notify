...
test("adapter probe workflow explains capture, interpretation, and privacy boundaries", () => {
  ...
  assert.match(workflow, /stableIgnoredCandidates/);
  assert.match(workflow, /scenarioCoverage/);
  assert.match(workflow, /recommendation/);
  assert.match(workflow, /handoffSummary/);
  assert.match(workflow, /adapterChecklist/);
  ...
  assert.match(workflow, /implementationNotes/);
